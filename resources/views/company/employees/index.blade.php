...
@section('content')
    <div class="container-fluid">
        ...
        <!-- FILTERS (company is fixed, so only dept + status) -->
        <div class="card mb-3">
            <div class="card-body">
                <form method="GET" action="{{ route('company.employees.index') }}" class="row g-3">
                    ...
                    <div class="col-12 text-end">
                        ...
                        <a href="{{ route('company.employees.index') }}" class="btn btn-light">Reset</a>
                    </div>
                    ...
                    @foreach ($employees as $employee)
                        @php
                            $avatarUrl = $employee->photo_path
                                ...
                                : asset('build/images/users/avatar-1.jpg'); // fallback image
                        @endphp
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex align-items-center">
                                    <img src="{{ $avatarUrl }}"
                                         alt="Avatar of {{ $employee->user->full_name }}"
                            ...
                            <td>
                                <div class="btn-group btn-group-sm">
                                    ...
                                    {{-- Attendance --}}
                                    <a href="{{ route('company.employees.attendance', $employee->id) }}"
                                       class="btn btn-success">
                                        Attendance
                                    </a>

                                    {{-- Schedules --}}
                                    <a href="{{ route('company.schedules.index', ['employee_id' => $employee->id]) }}"
                                       class="btn btn-primary">
                                        Schedules
                                    </a>
...
@section('scripts')
    ...
    <script>
        ...
        // Edit button handler (company routes)
        $(document).on('click', '.btn-edit-employee', function () {
            ...
            // Build update URL from named route
            const updateUrlTemplate = "{{ route('company.employees.update', ':id') }}";
            $('#editEmployeeForm').attr('action', updateUrlTemplate.replace(':id', id));

            // Fill fields (company is fixed, so no company_id field)
